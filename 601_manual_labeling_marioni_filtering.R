...
output_dir <- "601_manual_labeling_marioni_filtering"
...
process_condition <- function(condition) {
  ...
  if (length(pgc_cells) > 0) {
    p5 <- FeaturePlot(seurat_obj, features = naive_pluripotency_genes, ncol = 2) +
      ggtitle(paste0(toupper(condition), " - Naive Pluripotency Gene Expression"))
    print(p5)
  }
  ...
  p6 <- ggplot(cell_type_counts_df_before, aes(x = reorder(CellType, Count), y = Count)) +
    ...
    geom_text(aes(label = paste0(Count, " (", Percentage, "%)")), 
              hjust = -0.1, size = 3) +
    coord_flip() +
    labs(title = paste0(toupper(condition), " - Cell Type Composition Before Filtering"),
         x = "Cell Type",
         y = "Number of Cells") +
    theme_minimal() +
  ...
  png(file.path(output_dir, paste0("601_", condition, "_before_filtering.png")), 
  ...
  p4 <- ggplot(cell_type_counts_df_after, aes(x = reorder(CellType, Count), y = Count)) +
    ...
    geom_text(aes(label = paste0(Count, " (", Percentage, "%)")), 
              hjust = -0.1, size = 3) +
    coord_flip() +
    labs(title = paste0(toupper(condition), " - Cell Type Composition After Filtering"),
         x = "Cell Type",
         y = "Number of Cells") +
    theme_minimal() +
    theme(axis.text.y = element_text(size = 10),
  ...
  # Create PNG versions for presentation
  png(file.path(output_dir, paste0("601_", condition, "_after_filtering.png")), 
  ...
  pdf(file.path(output_dir, paste0("601_", condition, "_cell_type_composition.pdf")), 
      height = 10, width = 15)
  ...
  png(file.path(output_dir, paste0("601_", condition, "_cell_type_composition.png")), 
      height = 10*300, width = 15*300, res = 300)
  print(p)
  dev.off()
  
  # Save summary statistics
  write.csv(summary_stats, file.path(output_dir, paste0("601_", condition, "_summary_stats.csv")), 
            row.names = FALSE)
  write.csv(cell_type_composition_df, file.path(output_dir, paste0("601_", condition, "_cell_type_composition.csv")), 
            row.names = FALSE)
  
  # Save the filtered dataset
  saveRDS(seurat_obj_filtered, 
          file.path(output_dir, paste0("601_", condition, "_filtered.rds")))
...
write.csv(overall_summary, file.path(output_dir, "601_overall_summary.csv"), row.names = FALSE)

# Create combined cell type composition
all_compositions <- do.call(rbind, lapply(names(results), function(condition) {
  comp <- results[[condition]]$composition
  comp$Condition <- condition
  return(comp)
}))

write.csv(all_compositions, file.path(output_dir, "601_all_conditions_cell_type_composition.csv"), 
          row.names = FALSE)
...
foregut_analysis <- all_compositions %>%
  ...
  summarise(
    ...
    Thyroid_primordium = sum(Count[CellType == "Thyroid primordium"]),
...
for(i in 1:nrow(foregut_analysis)) {
  cond <- foregut_analysis$Condition[i]
...
for(i in 1:nrow(pgc_analysis)) {
  ...
  cat(sprintf("- %s: %d converted (%.1f%%), %d kept as PGC\n", 
              toupper(cond), converted, conversion_rate, pgc_kept))
}

# Save detailed merging analysis to CSV files
write.csv(foregut_analysis, file.path(output_dir, "601_foregut_merging_analysis.csv"), row.names = FALSE)
write.csv(cardiopharingeal_analysis, file.path(output_dir, "601_cardiopharingeal_analysis.csv"), row.names = FALSE)
write.csv(pgc_analysis, file.path(output_dir, "601_pgc_conversion_analysis.csv"), row.names = FALSE)

# Create combined visualization
pdf(file.path(output_dir, "601_all_conditions_comparison.pdf"), height = 20, width = 25)

# Plot 1: Cell counts comparison
p1 <- ggplot(overall_summary, aes(x = Condition, y = Cells_After, fill = Condition)) +
  geom_bar(stat = "identity", alpha = 0.8) +
  geom_text(aes(label = Cells_After), vjust = -0.5, size = 4) +
  labs(title = "Cell Counts After Filtering by Condition",
       x = "Condition",
       y = "Number of Cells") +
  theme_minimal() +
  theme(legend.position = "none")

# Plot 2: Cell type composition by condition
p2 <- ggplot(all_compositions, aes(x = Condition, y = Count, fill = CellType)) +
...
# Plot 3: Cell type percentage by condition
p3 <- ggplot(all_compositions, aes(x = Condition, y = Percentage, fill = CellType)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(title = "Cell Type Percentage by Condition",
       x = "Condition",
       y = "Percentage (%)",
       fill = "Cell Type") +
  theme_minimal() +
...
# Create PNG version for presentation
png(file.path(output_dir, "601_all_conditions_comparison.png"), 
    height = 20*300, width = 25*300, res = 300)
...
cat("Summary statistics saved in: 601_overall_summary.csv\n")
cat("Cell type compositions saved in: 601_all_conditions_cell_type_composition.csv\n")
cat("Detailed merging analysis saved in:\n")
cat("  - 601_foregut_merging_analysis.csv\n")
cat("  - 601_cardiopharingeal_analysis.csv\n")
cat("  - 601_pgc_conversion_analysis.csv\n")
cat("\nPNG files for presentation created:\n")
cat("  - 601_{condition}_before_filtering.png\n")
cat("  - 601_{condition}_after_filtering.png\n")
cat("  - 601_{condition}_cell_type_composition.png\n")
cat("  - 601_all_conditions_comparison.png\n")
